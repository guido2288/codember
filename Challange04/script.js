const unchecksum = (codigo) => {

  const cadena = codigo.split("-")[0];
  const validation = codigo.split("-")[1];

  let resp = [];

  cadena.split('').forEach(element => {

    validation.split('').forEach(letter => {
      if(element == letter){
        resp.push(element)
      }
  ...
  for (let i = 0; i < respuestafinal.length; i++) {
    if(respuestafinal[i] !== validation.split('')[i]){
      return false
    }
...
console.log( unchecksum("O2hrQ-O2hrQ"));