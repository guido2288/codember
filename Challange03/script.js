const checkPass = (pass) => {
  ...
  for (const element of pass) {
    ...
    const politica = element.split(":")[0];
    const clave = element.split(":")[1].trim().split("");
    const caracter = politica[politica.length - 1];
  
    const min = Number( politica.split("-")[0]);
    const max = Number (politica.split("-")[1].split(" ")[0]);

    const cantidadCaracteres = clave.filter(letter => letter == caracter).length;

    if(cantidadCaracteres >= min && cantidadCaracteres <=max) {
